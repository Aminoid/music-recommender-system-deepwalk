...
def process(args):
    ...
    deepwalk = graph.build_deepwalk_corpus(G, args.number_walks,
                args.walk_length, alpha=0, rand=random.Random(0))
    model = Word2Vec(deepwalk, size=args.representation_size, 
...
def predict_rating(model, nodedict, user, movie):
    ...
    for i in range(1, 6):
        mvr = nodedict[movie + '-' + str(i)].id
        s = model.similarity(uid, mvr)
        if s > max_s:
            max_s = s
            r = i
